...
//  Modified from https://github.com/lukedeo/AGILEPack/tree/release/dataframe
//-----------------------------------------------------------------------------
    ...
    if (filename != "")
    {
        ...
        if (colnames)
        {
            ...
            while (getline( ss, field, ',' ))
            {
                ...
                column_names[ds::no_quotes(ds::trim(fs.str()))] = ctr;
                ++ctr;
            }
        }
        input >> data;
...
std::vector<std::string> dataframe::get_column_names()
{
    ...
    {
        v.at(var.second) = ds::trim(var.first);
    }
    return std::move(v);
}
...
statvector dataframe::at(const std::string &colname) 
{
    auto column_id = dataframe::get_column_idx(colname);
    record_t feature;
    for (auto &record : data)
    {
        feature.push_back(record.at(column_id));
    }
    return std::move(ds::statvector(feature));
}
